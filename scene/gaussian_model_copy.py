...
class GaussianModel:
	...
	def save_ply(self, path):
		...
		try:
			np.savetxt(os.path.join(os.path.split(path)[0], "points_per_obj.txt"), np.array([x for x in self.points_per_obj]))
		except:
			np.savetxt(os.path.join(os.path.split(path)[0], "points_per_obj.txt"), np.array([x.cpu() for x in self.points_per_obj]))
	...
	def reset_opacity_selective(self, obj):
		opacities_new = []
		l = 0
		for i in range(self.num_objs):
			if i == obj:
				...
				opacities_new.append(self.get_opacity[i, :self.points_per_obj[i]])
			l = max(l, self.points_per_obj[i])
		opacities_new = self.zero_pad_tensor(opacities_new, l)
		optimizable_tensors = self.replace_tensor_to_optimizer(opacities_new, "opacity")
		self._opacity = optimizable_tensors["opacity"]
	...
	def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
		...
		for i in range(self.num_objs):
			...
			try:
				new_points_per_obj.append(int(new_xyz[i].shape[0].cpu()))
			except:
				new_points_per_obj.append(int(new_xyz[i].shape[0]))
	...
	def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size):
		...
		for i in range(self.num_objs):
			...
			length.append(int(prune_mask[i].sum().cpu()))
		...
		for group in self.optimizer.param_groups:
			if group["name"] not in ['background']:
				...
				if stored_state is not None:
					...
					del self.optimizer.state[group['params'][0]]
					group["params"][0] = nn.Parameter(self.concatenate_reinit_tensors(extension_tensor).requires_grad_(True))
					self.optimizer.state[group['params'][0]] = stored_state
	...
	def reinit_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation):
		d = {"xyz": new_xyz,
		"f_dc": new_features_dc,
		"f_rest": new_features_rest,
		"opacity": new_opacities,
		...
		optimizable_tensors = self.cat_reinit_tensors_to_optimizer(d)
		self._xyz = optimizable_tensors["xyz"]
		self._features_dc = optimizable_tensors["f_dc"]
		self._features_rest = optimizable_tensors["f_rest"]
		self._opacity = optimizable_tensors["opacity"]
	...
	def reinit_from_state_dict(self, model_args, obj):
		new_xyz = []
		new_features_dc = []
		new_features_rest = []
		new_opacities = []
		...
		for i in range(self.num_objs):
			if i != obj:
				new_xyz.append(_xyz[i, :points_per_obj[i]])
				new_features_dc.append(_features_dc[i, :points_per_obj[i]])
				new_features_rest.append(_features_rest[i, :points_per_obj[i]])
				...
				new_features_rest.append(self._features_rest[i, :self.points_per_obj[i]])
