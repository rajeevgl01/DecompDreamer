...
class StableDiffusion(nn.Module):
	def __init__(self, device, fp16, vram_O, t_range=[0.02, 0.98], max_t_range=0.98, num_train_timesteps=None,
		...
		base_model_key = "stabilityai/stable-diffusion-2-1-base" if guidance_opt.base_model_key is None else guidance_opt.base_model_key
		...
		if use_control_net:
			...
			self.controlnet_depth = ControlNetModel.from_pretrained(
				controlnet_model_key, torch_dtype=self.precision_t).to(device)
			for p in self.controlnet_depth.parameters():
				p.requires_grad_(False)
			self.controlnet_depth = self.controlnet_depth.eval()
	...
	def train_step_perpneg(self, text_embeddings, pred_rgb, pred_depth=None, pred_alpha=None,
		...
		with torch.no_grad():
			...
			if use_control_net:
				pred_depth_input = pred_depth[None, :, ...].repeat(
					1 + K, 1, 3, 1, 1).reshape(-1, 3, 512, 512).half()
				...
				unet_output = self.unet(latent_model_input.to(self.precision_t), tt.to(
					self.precision_t), encoder_hidden_states=text_embeddings.to(self.precision_t)).sample
	...
	def train_step(self, text_embeddings, pred_rgb, pred_depth=None, pred_alpha=None,
		...
		pred_rgb, pred_depth_, pred_alpha = self.augmentation(
		...
		if as_latent:
			latents, _ = self.encode_imgs(
				pred_depth_.repeat(1, 3, 1, 1).to(self.precision_t))
		else:
			latents, _ = self.encode_imgs(pred_rgb.to(self.precision_t))
		# timestep ~ U(0.02, 0.98) to avoid very high/low noise level
		...
		if iteration % guidance_opt.vis_interval == 0:
			...
			with torch.no_grad():
				...
				viz_images = torch.cat([pred_rgb,
										...
										rgb2sat(pred_rgb, pred_alpha).repeat(
											1, 3, 1, 1),
