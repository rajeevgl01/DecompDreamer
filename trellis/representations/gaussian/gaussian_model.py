...
class Gaussian:
	...
	def load_ply(self, path, num_pts):
		...
		if self.sh_degree > 0:
			...
			# Ensure the check aligns with your SH degree config
		...
		# FIX 1: REMOVE SIGMOID
		# Your PLY stores 'inverse_sigmoid' (logits). Loading them as-is is correct
		# because you pass them to 'inverse_opacity_activation' (or assign to _opacity) later.
		opacities = torch.tensor(opacities, dtype=torch.float, device=self.device)
		
		# FIX 2: REMOVE EXP (This fixes the OOM/Scale issue)
		# Your PLY stores 'log(scale)'. self._scaling expects 'log(scale)'.
		...
		# Since opacities are already logits (from PLY), we just remove bias
		self._opacity = opacities - self.opacity_bias
		
		# Assign scales directly (they are already log-space)
		self._scaling = scales
		self._rotation = rots - self.rots_bias[None, :]

		# if self._xyz.shape[0] > num_pts:
		indices = torch.randperm(self._xyz.shape[0], device=self._xyz.device)[:num_pts]
