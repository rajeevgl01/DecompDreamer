...
from gaussian_renderer import render_main, render, render_obj, network_gui
import sys
...
def forward(opt, embeddings, objs, azimuth_offset, iteration, viewpoint_stack, scene, guidance_opt, debug_from, gaussians, pipe, background, dataset, guidance, use_control_net, save_folder, gcams):
	...
	cam_scale = gcams.radius_params[objs[0]] if len(objs) == 1 else 1.0
	# Pick a random Camera
	if not viewpoint_stack:
		viewpoint_stack_raw, forward_vector_raw = scene.getRandTrainCameras(cam_scale)
		viewpoint_stack = viewpoint_stack_raw.copy()
		forward_vector = forward_vector_raw.copy()
	...
	for i in range(C_batch_size):
		...
		render_pkg = render_main(viewpoint_cam, view_dir, gaussians, pipe, background, objs,
...
def training(dataset, opt, pipe, gcams, guidance_opt, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, save_video, cfg):
	...
	for iteration in range(first_iter, opt.iterations + 1):
		...
		gaussians.update_pos_delta_learning_rate(iteration)
